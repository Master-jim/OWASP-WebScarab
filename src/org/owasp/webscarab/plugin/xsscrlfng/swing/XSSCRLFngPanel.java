...
public class XSSCRLFngPanel extends javax.swing.JPanel implements SwingPluginUI {
	...
	*/
	...
	private ArrayList<ColumnDataModel<ConversationID>> _vulnerableConversationColumns;
	
	private ArrayList<ColumnDataModel<HttpUrl>> _vulnerableUrlColumns;
	...
	// 2013-03-07 - JLS - Needed to convert ArrayList<ColumnDataModel<ConversationID>> to an array - BEGIN
	private static final ColumnDataModel[] CDM = new ColumnDataModel[0];
	// 2013-03-07 - JLS - Needed to convert ArrayList<ColumnDataModel<ConversationID>> to an array - END
	...
	public XSSCRLFngPanel(XSSCRLFng xsscrlf) {
		...
		_vulnerableConversationColumns = new ArrayList<ColumnDataModel<ConversationID>>(4);
		ConversationTableModel vtm = new ConversationTableModel(_model.getVulnerableConversationModel());

		_vulnerableConversationColumns.add(		
			new ColumnDataModel<ConversationID>("XSS-ng", Boolean.class) {
				public Object getValue(ConversationID key) {
					if (_model.getXSSParamVulnerableInjected(key) != null) {
						return (_model.getXSSParamVulnerableInjected(key).length() > 0);
					}
		...
		vtm.addColumn(_vulnerableConversationColumns.get(0));
		
		vtm.addColumn(_vulnerableConversationColumns.get(1));
		
		vtm.addColumn(_vulnerableConversationColumns.get(2));
		
		ConversationTableModel stm = new ConversationTableModel(_model.getTestedConversationModel());

		// 2011-03-15 - JLS - Modifying rows showed in XSS panel - BEGIN
		stm.addColumn(_vulnerableConversationColumns.get(0));
		...
		_vulnerableUrlColumns = new ArrayList<ColumnDataModel<HttpUrl>>(2);
/*		_vulnerableUrlColumns.add(
		...
		_vulnerableUrlColumns.add(
			new ColumnDataModel<HttpUrl>("Injection-ng", Boolean.class) {
				public Object getValue(HttpUrl key) {
					HttpUrl url = key;
					return _model.isXSSVulnerable(url)? Boolean.TRUE :  Boolean.FALSE;
				}
			});
	...
	private void initComponents() {
		...
		if (! _xsscrlf.getIsTestingAllRequests()) {
		...
		// 2011-07-26 - JLS - Adding a button for SQLi tests - BEGIN
		if (! _xsscrlf.isPerformingSQLiTests()) {
	...
	private void okButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_okButtonActionPerformed
		...
		// 2011-07-05 - JLS - Adding a text area for the XSS url - END
		// 2011-03-14 - JLS - Adding a text area for the XSS search functions - BEGIN
		// TEMPFIX //_xsscrlf.setXSSPatternToFind(patternCompleteTextArea.getText());
		// TEMPFIX //_xsscrlf.setXSSPatternToFindPartial(patternPartialTextArea.getText());
		// 2011-03-14 - JLS - Adding a text area for the XSS search functions - END
	...
	private void loadButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_loadButtonActionPerformed
		...
		jfc.setDialogTitle("Open  string file");
		int returnVal = jfc.showOpenDialog(this);
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			File extFile = jfc.getSelectedFile();
			// TEMPFIX //try {
				String testString=""; // TEMPFIX //_xsscrlf.loadString(extFile);
			...
			// TEMPFIX //}
			// TEMPFIX //} catch (IOException ioe) {
			// TEMPFIX //	JOptionPane.showMessageDialog(null, new String[] {"Error loading test string: ", ioe.getMessage()}, "Error", JOptionPane.ERROR_MESSAGE);
			// TEMPFIX //}
		}
	...
	private void editButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_editButtonActionPerformed
		xssTextArea.setText(_model.getXSSTestStrings());
		// 2011-07-05 - JLS - Adding a text area for the XSS url - BEGIN
		xssUrlFilter.setText(_xsscrlf.getUrlOfTarget());
		// 2011-07-05 - JLS - Adding a text area for the XSS url - END
		// TEMPFIX //patternCompleteTextArea.setText(_xsscrlf.getXSSPatternToFind());
		// TEMPFIX //patternPartialTextArea.setText(_xsscrlf.getXSSPatternToFindPartial());
	...
	private void checkButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_checkButtonActionPerformed
		...
		new SwingWorker() {
			public Object construct() {                
				// TEMPFIX //_xsscrlf.checkSelected(CIDs);
				return null;
	...
	private void activateButtonActionPerformed(java.awt.event.ActionEvent evt) {
		new SwingWorker() {
			public Object construct() {
				if (! _xsscrlf.getIsTestingAllRequests()) {
					activateButton.setText("Activate");
					_xsscrlf.testAllRequests(Boolean.FALSE);
				} else {
					activateButton.setText("Deactivate");
					_xsscrlf.testAllRequests(Boolean.TRUE);
				}
				return null;
			...
			public Object construct() {
				...
				for (int i=0 ; i<count; i++) {
					...
					if (id != null) {
						...
						// TEMPFIX //_xsscrlf.analyseForced(id, conversationModel.getRequest(id), conversationModel.getResponse(id), _xsscrlf.getPluginName(), Boolean.TRUE);
					} else {
					...
					for (int i=0 ; i<count; i++) {
						if (!reCheckIsFinishedOrAborted) {
							...
							if (id != null) {
								...
								// TEMPFIX //_xsscrlf.responseReceivedWithReanalyse(conversationModel.getResponse(id), true);
							} else {
								_logger.severe ("No id found for the Conversation : "+i);
							}
	...
	public ColumnDataModel<ConversationID>[] getConversationColumns() {
		return (ColumnDataModel<ConversationID>[]) _vulnerableConversationColumns.toArray(CDM);
	}
	...
	public ColumnDataModel<HttpUrl>[] getUrlColumns() {
		return (ColumnDataModel<HttpUrl>[]) _vulnerableUrlColumns.toArray(CDM);
	}
	...
	private class XSSngTestConversation extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			_xsscrlf.checkConversation((ConversationID) o);
